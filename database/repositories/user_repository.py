...
class UserRepository:
    ...
    async def find_by_username(self, username: str) -> Optional[User]:
        ...
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT id, username, password, role, chat_id FROM users WHERE username = $1",
                username,
            )
            if row is None:
                return None
    ...
    async def create(self, username: str, password: str, role: str, chat_id: Optional[int] = None) -> User:
        """Create a new user.
        ...
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                """
                INSERT INTO users (username, password, role, chat_id)
                VALUES ($1, $2, $3, $4)
                RETURNING id, username, password, role, chat_id
                """,
                username, password, role, chat_id,
            )
            return User(
                id=row["id"],
                username=row["username"],
    ...
    async def get_all_staff(self) -> list[User]:
        ...
        async with self._pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT id, username, password, role, chat_id FROM users ORDER BY id"
            )
            return [
                User(
                    id=row["id"],
    ...
    async def get_all_waiters(self) -> list[User]:
        ...
        async with self._pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT id, username, password, role, chat_id "
                "FROM users WHERE role = 'waiter' ORDER BY id"
            )
            return [
                User(
                    id=row["id"],
    ...
    async def find_by_id(self, user_id: int) -> Optional[User]:
        ...
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT id, username, password, role, chat_id FROM users WHERE id = $1",
                user_id,
            )
            if row is None:
                return None
    ...
    async def ensure_admin_exists(self, username: str, password: str) -> None:
        """Ensure that an admin account exists, create if not.
